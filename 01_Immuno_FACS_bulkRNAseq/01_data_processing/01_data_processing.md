...
Genome indexing was performed using **STAR** in `genomeGenerate` mode. The index was built with 8 threads using the genome FASTA file and modified GTF annotation, with splice junctions generated using a read-length–appropriate overhang (`sjdbOverhang = 84`). The resulting indexed genome was used for downstream RNA-seq read alignment.
